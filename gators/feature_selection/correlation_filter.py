...
class CorrelationFilter(_BaseFeatureSelection):
    ...
    * fit with `koalas` & transform with `NumPy`
    ...
    def fit(self,
            ...
            y: Union[pd.Series, ks.Series] = None) -> 'CorrelationFilter':
        ...
        if not isinstance(X, pd.DataFrame):
            corr = corr.to_pandas()
        stacked_corr = (corr.where(np.tril(np.ones(corr.shape), k=-1).astype(np.bool))
                        .stack()
                        .sort_values(ascending=False))
        stacked_corr = stacked_corr.sort_values(ascending=False)
        mask = (stacked_corr >= self.max_corr)
        self.columns_to_drop = stacked_corr[mask].sort_index(
        ).index.get_level_values(1)
        self.columns_to_drop = list(set(self.columns_to_drop))
        self.selected_columns = [
            c for c in columns if c not in self.columns_to_drop]
        self.feature_importances_ = pd.Series(
            1., index=self.selected_columns, dtype=float)
