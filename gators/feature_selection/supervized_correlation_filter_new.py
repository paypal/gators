...
class SupervizedCorrelationFilterNew(_BaseFeatureSelection):
    ...
    * and `pandas` dataframes:
    ...
    def __init__(
        self, feature_importances: Series, corr: pd.DataFrame, max_corr: float
    ):
    ...
    def fit(self, X: DataFrame, y: Series = None) -> "SupervizedCorrelationFilterNew":
        ...
        for i in range(len(columns)):
            for j in range(i + 1):
                item = self.corr.iloc[j : (j + 1), (i + 1) : (i + 2)]
                col = item.columns
                row = item.index
                val = item.values
