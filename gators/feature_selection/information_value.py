...
def compute_iv(X, y, regularization=0.01):
    ...
    └──────────┴────────────┘
    """
    string_cat_cols = [col for col, dtype in X.schema.items() if dtype in [pl.String, pl.Categorical]]
    
    # Return empty DataFrame if no String/Categorical columns
    if not string_cat_cols:
        return pl.DataFrame({"feature": [], "iv": []}, schema={"feature": pl.String, "iv": pl.Float64})
