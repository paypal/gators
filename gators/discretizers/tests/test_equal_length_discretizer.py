...
def test_as_numerics(sample_data):
    ...
    assert transformed_X["A__discretize_length"].dtype == pl.Float64
    # Labels should be numeric values 0, 1, 2
    unique_labels = transformed_X["A__discretize_length"].unique().sort().to_list()
    # Should have numeric labels
    assert all(label in [0.0, 1.0, 2.0] for label in unique_labels)
