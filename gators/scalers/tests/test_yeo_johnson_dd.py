...
def compute_iv(X, y, regularization=0.1):
    y_name = y.name
    mask = X.dtypes == object
    object_columns = mask[mask].index
    stats = (
        pd.melt(X[object_columns].join(y), id_vars=y_name)
    ...
    iv = (stats["distrib_1"] - stats["distrib_0"]) * np.log(
        stats["distrib_1"] / stats["distrib_0"]
    )
