...
class TestRobustScaler:
    ...
    def test_median_is_zero_after_scaling(self, X):
        scaler = RobustScaler(subset=["a", "b"])
        scaler.fit(X)
        result = scaler.transform(X)
    ...
    def test_scaled_iqr_is_one(self, X):
        scaler = RobustScaler(subset=["a", "b"], quantile_range=(0.25, 0.75))
        scaler.fit(X)
        result = scaler.transform(X)
    ...
    def test_subset_only_scales_selected_columns(self, X):
        scaler = RobustScaler(subset=["a"], drop_columns=False)
        scaler.fit(X)
        result = scaler.transform(X)
        assert "a__robust_quantile_scale" in result.columns
    ...
    def test_custom_quantile_range(self, X):
        scaler = RobustScaler(subset=["a"], quantile_range=(0.1, 0.9))
        scaler.fit(X)
        result = scaler.transform(X)
        assert "a__robust_quantile_scale" in result.columns
    ...
    def test_get_params(self):
        scaler = RobustScaler(quantile_range=(0.1, 0.9), drop_columns=False)
    ...
    def test_values_correctness(self):
        ...
        scaler = RobustScaler(subset=["x"], quantile_range=(0.25, 0.75))
        scaler.fit(X)
        result = scaler.transform(X)
