...
def data_ks():
    ...
    X_int_ks = ps.from_pandas(X_int)
    X_float_ks = ps.from_pandas(X_float)
    X_object_ks = ps.from_pandas(X_object)
...
@pytest.fixture()
def data_num_ks():
    X_int = ps.DataFrame({"A": [0, 1, 1, np.nan], "B": [3, 4, 4, np.nan]})
    X_float = ps.DataFrame({"C": [0.1, 1.1, 2.1, np.nan], "D": [2.1, 3.1, 4.1, np.nan]})
    X_int_expected = pd.DataFrame(
        {"A": [0.0, 1.0, 1.0, -9.0], "B": [3.0, 4.0, 4.0, -9.0]}
    )
    X_float_expected = pd.DataFrame(
...
def data_no_missing_ks():
    X_int = ps.DataFrame({"A": [0, 1, 1, 8], "B": [3, 4, 4, 8]}, dtype=int)
    X_float = ps.DataFrame({"C": [0.1, 1.1, 2.1, 9.0], "D": [2.1, 3.1, 4.1, 9.0]})
    X_object = ps.DataFrame({"E": ["q", "w", "w", "x"], "F": ["a", "a", "s", "x"]})
    obj_int = NumericImputer(strategy="constant", value=-9, columns=list("AB")).fit(
        X_int
    )
    obj_float = NumericImputer(strategy="mean", columns=list("CD")).fit(X_float)
...
def data_full_ks():
    ...
    X = ps.from_pandas(pd.concat([X_int, X_float, X_object], axis=1))
