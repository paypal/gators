...
def data():
    X = pd.DataFrame(
        [[200.0, 140.0, 100.0], [210.0, 160.0, 125.0]],
        columns=list('XYZ'))
    ...
    obj = PlaneRotation(
        columns=[['X', 'Y'], ['X', 'Z']], theta_vec=[45, 60]).fit(X)
...
def data_float32():
    X = pd.DataFrame(
        [[200, 140, 100], [210, 160, 125]],
        columns=list('XYZ')).astype(np.float32)
    ...
    obj = PlaneRotation(
        columns=[['X', 'Y'], ['X', 'Z']], theta_vec=[45, 60],
