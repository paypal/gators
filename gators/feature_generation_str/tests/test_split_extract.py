...
def data():
    ...
    obj = SplitExtract(
        list('asd'), list('***'), [1, 1, 0]).fit(X)
    columns_expected = [
        'q', 'w', 'e', 'a', 's', 'd', 'a__split_by_*_idx_1',
        's__split_by_*_idx_1', 'd__split_by_*_idx_0']
...
def test_init():
    with pytest.raises(TypeError):
        _ = SplitExtract(
            columns='x', str_split_vec=['+', '-'], idx_split_vec=[1, 2]
        )
    with pytest.raises(TypeError):
        _ = SplitExtract(
            columns=['a', 's'], str_split_vec='+', idx_split_vec=[1, 2]
        )
    with pytest.raises(TypeError):
        _ = SplitExtract(
            columns=['a', 's'], str_split_vec=['+', '-'], idx_split_vec=0
        )
