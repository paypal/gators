...
def data():
    ...
    X['d'] = ['0', 'Q', '']
    obj = Extract(
        columns=list('asd'), i_min_vec=[0, 1, 2], i_max_vec=[1, 2, 3]).fit(X)
    columns_expected = [
        'q', 'w', 'e', 'a', 's', 'd', 'a__substring_0_to_1',
        's__substring_1_to_2', 'd__substring_2_to_3']
...
def test_drop_low_cardinality_init(data):
    with pytest.raises(TypeError):
        _ = Extract(
            columns='x', i_min_vec=[0, 1], i_max_vec=[1, 2]
        )
    with pytest.raises(TypeError):
        _ = Extract(
            columns=['a', 's'], i_min_vec=0, i_max_vec=[1, 2]
        )
    with pytest.raises(TypeError):
        _ = Extract(
            columns=['a', 's'], i_min_vec=[0, 1], i_max_vec=0
        )
    ...
    with pytest.raises(ValueError):
        _ = Extract(
            columns=['a', 's'], i_min_vec=[0, 1], i_max_vec=[1]
        )
