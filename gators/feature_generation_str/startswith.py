...
from feature_gen_str import startswith
...
class Startswith(_BaseStringFeature):
    """Create new binary columns.

    The value is 1 if the element startswith the given substring and 0 otherwise.
    ...
    * and `pandas` dataframes:
    ...
    def __init__(
        ...
        if not column_names:
            column_names = [
                f"{col}__startswith_{val}" for col, val in zip(columns, pattern_vec)
            ]
    ...
    def transform(self, X: DataFrame) -> DataFrame:
        ...
        for col, val, name in zip(self.columns, self.pattern_vec, self.column_names):
            X[name] = X[col].str.startswith(val).astype(np.float64)
    ...
    def transform_numpy(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        self.check_array(X)
        return startswith(X, self.idx_columns, self.pattern_vec_np)
