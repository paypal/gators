...
class TestMapSubstringReplacements:
    ...
    def test_basic_space_to_underscore(self):
        ...
        X = pl.DataFrame(
            {"categories": ["cat A", "cat B", "cat A", "dog C"], "numbers": [1, 2, 3, 4]}
        )

        mapping = map_substring_replacements(X, old=" ", new="_")

        expected = {"categories": {"cat A": "cat_A", "cat B": "cat_B", "dog C": "dog_C"}}
        assert mapping == expected

    def test_multiple_string_columns(self):
        """Test with multiple string columns."""
        X = pl.DataFrame(
            {"col1": ["value A", "value B"], "col2": ["item X", "item Y"], "col3": [1, 2]}
        )
    ...
    def test_no_matches_returns_empty_dict(self):
        ...
        X = pl.DataFrame({"no_spaces": ["valueA", "valueB", "valueC"], "numbers": [1, 2, 3]})
    ...
    def test_only_numeric_columns_returns_empty(self):
        ...
        X = pl.DataFrame(
            {"int_col": [1, 2, 3], "float_col": [1.1, 2.2, 3.3], "bool_col": [True, False, True]}
        )
    ...
    def test_mixed_columns_only_processes_strings(self):
        ...
        expected = {"strings": {"a b": "a_b", "c d": "c_d"}}
        assert mapping == expected

    def test_categorical_column(self):
        """Test with categorical dtype column."""
        X = pl.DataFrame({"cat_col": ["cat A", "cat B", "cat A"]}).with_columns(
            pl.col("cat_col").cast(pl.Categorical)
        )

        mapping = map_substring_replacements(X, old=" ", new="_")

        expected = {"cat_col": {"cat A": "cat_A", "cat B": "cat_B"}}
        assert mapping == expected

    def test_duplicate_values_in_column(self):
        """Test that duplicate values are handled correctly (unique values only)."""
        X = pl.DataFrame({"col": ["item A", "item B", "item A", "item B", "item C"]})

        mapping = map_substring_replacements(X, old=" ", new="_")

        expected = {"col": {"item A": "item_A", "item B": "item_B", "item C": "item_C"}}
        assert mapping == expected

    def test_different_old_new_characters(self):
        """Test with different old and new substrings."""
        X = pl.DataFrame({"col1": ["a-b", "c-d"], "col2": ["x.y", "z.w"]})
    ...
    def test_empty_dataframe(self):
        ...
        X = pl.DataFrame({"col1": [], "col2": []}).with_columns(
            [pl.col("col1").cast(pl.String), pl.col("col2").cast(pl.Int64)]
        )
    ...
    def test_column_with_nulls(self):
        ...
        X = pl.DataFrame({"col": ["value A", None, "value B", None]})
    ...
    def test_partial_match_only_some_values(self):
        ...
        X = pl.DataFrame({"col": ["value A", "valueB", "value C", "valueD"]})
    ...
    def test_multiple_occurrences_in_same_value(self):
        ...
        X = pl.DataFrame({"col": ["a b c", "x y z"]})
    ...
    def test_replace_with_empty_string(self):
        ...
        X = pl.DataFrame({"col": ["value A", "value B"]})
    ...
    def test_multicharacter_old_and_new(self):
        ...
        X = pl.DataFrame({"col": ["value__A", "value__B", "item__C"]})

        mapping = map_substring_replacements(X, old="__", new="--")

        expected = {"col": {"value__A": "value--A", "value__B": "value--B", "item__C": "item--C"}}
        assert mapping == expected

    def test_case_sensitive_matching(self):
        """Test that matching is case-sensitive."""
        X = pl.DataFrame({"col": ["Value A", "value a", "VALUE A"]})

        mapping = map_substring_replacements(X, old=" ", new="_")

        # All should be replaced (case doesn't matter for space)
        expected = {"col": {"Value A": "Value_A", "value a": "value_a", "VALUE A": "VALUE_A"}}
        assert mapping == expected

    def test_unicode_characters(self):
        """Test with unicode characters."""
        X = pl.DataFrame({"col": ["café au lait", "crème brûlée"]})

        mapping = map_substring_replacements(X, old=" ", new="_")

        expected = {"col": {"café au lait": "café_au_lait", "crème brûlée": "crème_brûlée"}}
        assert mapping == expected
