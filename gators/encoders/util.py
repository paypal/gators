...
def determine_encoding_strategy(
    train: pl.DataFrame, max_count_woe: int = 100
) -> Tuple[list, list]:
    """
    ...
    """
    string_columns = [
        col for col, dtype in zip(train.columns, train.dtypes) if dtype in [pl.Categorical, pl.String]
    ]

    column_counts = {col: train[col].n_unique() for col in string_columns}
    woe_columns = [
        col for col, count in column_counts.items() if count <= max_count_woe
    ]
    onehot_columns = [
        col for col, count in column_counts.items() if count > max_count_woe
    ]
